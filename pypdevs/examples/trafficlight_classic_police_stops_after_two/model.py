...
class Policeman(AtomicDEVS):
    ...
    def intTransition(self):
        ...
        observed_four_red_lights = self.parent.getContextInformation("red") % 2 == 0
        print("[debug] Observed redligts %d" % self.parent.getContextInformation("red") )
        if state == "idle" or not observed_four_red_lights:
            return PolicemanMode("working")
        elif state == "working" and observed_four_red_lights:
...
class TrafficSystem(CoupledDEVS):
    ...
    def saveChildrenState(self, state):
        super(TrafficSystem, self).saveChildrenState(state)
        state_value = state[0].get()
        self._children_states[state_value] = self._children_states.get(state_value, 0) + 1


    def getContextInformation(self, property):
        super(TrafficSystem, self).getContextInformation(property)
        return self._children_states.get(property, 0)
