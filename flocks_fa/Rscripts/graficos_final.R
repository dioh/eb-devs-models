...
# model.eb = read_csv("/home/dfoguelman/proj/crazydevs/results/experiment_final.csv", 
model.eb = read_csv("/home/dfoguelman/proj/crazydevs/results/experiment_cicles_12_retries_10_duration_3600_dradius_ok_paper_version.csv",
  ...
  rename(t=currenttime) %>% select(-c(state, duration)) %>% add_column(model='EB-DEVS', duration=3600)

model.nl.new = read_csv("/home/dfoguelman/proj/crazydevs/results/netlogo_results_fuse_model.csv",
...
model.nl.old = read_csv("/home/dfoguelman/proj/crazydevs/results/netlogo_results_fuse_all.csv",
                        col_types = cols(large=col_double(), 
                                         medium=col_double(),
                                         small=col_double(),
                                         retry=col_number(),
...
# model.nl.old %>% rbind(model.nl.new) %>% #filter(t<6000) %>% 
...
#   geom_point(aes(y=mperc, color=model))  + facet_wrap(~ fissionprob)
...
model.eb %>% rbind(model.nl.new) %>% #filter(retry< 5) %>% 
  ggplot(data=.,
         aes(x=t, y=perc)) +
...
ggsave("~/proj/crazydevs/plots/results_mito_ebdevs_vs_nlogo_f18.png", width = 12, height = 8)

# Three subplots one by fission prob


# model.nl.new %>% rbind(model.eb) %>%
#   transform(mass_group = str_to_title(mass_group)) %>%
#   filter(t %% 10 ==0) %>%
#   unite(model_by_group, c(mass_group, model), sep=' ') %>% he
# ggplot(data=.,
#        aes(x=t, y=perc)) +
#   facet_grid( fissionprob ~ mass_group,
