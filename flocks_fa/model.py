...
class BirdState(object):
    ...
    def __init__(self, name, id):
        ...
        grid_side_width = ((GRID_SIZE ** 2) / FLOCK_SIZE) ** (0.5)

        grid_side_amount = GRID_SIZE / grid_side_width
...
class Bird(AtomicDEVS):
    ...
    def intTransition(self):
        ...
        self.state.neighbors = len(neighbors)

        if FLOCKS_AMOUNT > -1 and SUPERCOHERE_CAP > -1:
            assert False, "Super coherence and anti coherence should not happen at the same time"

        if (gettos < FLOCKS_AMOUNT  or gettos < SUPERCOHERE_CAP) and gettos > 1:
            # Reverse direction if flock relative size is bigg
            if FLOCKS_AMOUNT > -1:
                self.anticohere(neighbors)
            elif SUPERCOHERE_CAP > -1:
                self.supercohere(neighbors)
        else:
            if distance.euclidean(np.asarray(closest_coord),
                    np.asarray(self.state.coord)) < MAX_DIST:
    ...
    def separate(self, closest):
        ...
        turn_at_most = min(abs(delta), MAX_SEPARATE_TURN)
    ...
    def align(self, neighbors):
        ...
        turn_at_most = min(abs(delta), MAX_ALIGN_TURN)
    ...
    def supercohere(self, neighbors):
        global MAX_COHERE_TURN
        print("MAX COHERE TURN NOW IS %f" % MAX_COHERE_TURN)
        # MAX_COHERE_TURN = MAX_COHERE_TURN + 0.03
        self.cohere(neighbors)
    ...
    def cohere(self, neighbors):
        ...
        delta = new_heading - self.state.heading
        turn_at_most = min(abs(delta), MAX_COHERE_TURN)

        self.state.heading = self.state.heading + np.sign(delta) * turn_at_most

    def advance(self, step=STEP_DIST):
        ...
        self.state.coord[0] = (self.state.coord[0] + np.math.cos(rads) * step) % GRID_SIZE
        self.state.coord[1] = (self.state.coord[1] + np.math.sin(rads) * step) % GRID_SIZE
...
class Flock(CoupledDEVS):
    def __init__(self, name=None):
        ...
        # The TrafficLight 
        self.birds = [Bird(name="bird %d" % i, id=i)  for i in range(FLOCK_SIZE)]
        for bird in self.birds:
            self.addSubModel(bird)

        self.history = pd.DataFrame([[x.state.name, x.state.coord[0], x.state.coord[1], x.state.heading,
                        -1.5, None, 0] for x in self.birds],
                        columns=['bird', 'x', 'y', 'heading', 't', 'cluster', 'neighbors'])
    ...
    def saveChildrenState(self, state):
        ...
        if self.updatecount % FLOCK_SIZE == 0:
            # All birds updated, lets recompute.
            self.points = np.array([ np.array(x.coord) for x in self.states.values() ])

            self.model = NearestNeighbors(radius=RADIUS)
            self.model.fit(self.points)

            self.updatecount = 0
            if DEBUG:
                df_data = pd.DataFrame([[x.name, x.coord[0], x.coord[1], x.heading,
                                state[1][0], None, x.neighbors] for i, x in enumerate(self.states.values())] ,
                                columns=['bird', 'x', 'y', 'heading', 't', 'cluster', 'neighbors'])
                adj_mat = self.model.radius_neighbors_graph().toarray()
                gr = nx.from_numpy_matrix(adj_mat)
    ...
    def getContextInformation(self, property, *args, **kwargs):
        ...
        if property == "closest":
            coord = kwargs['coord']
            neighbors = self.model.radius_neighbors([coord])
            ...
            i_closest_in_res, = np.where(distances == min_not_zero)


            #i_closest_in_res = np.argmin(distances[np.nonzero(distances)])
            i_in_points_array = neighbors[1][0][i_closest_in_res[0]]
            return_point = list(self.states.values())[i_in_points_array]
            return return_point

        if property == "neighbors":
            coord = kwargs['coord']
            neighbors = self.model.radius_neighbors([coord], RADIUS)
            distances = neighbors[0][0]
            indexes = neighbors[1][0][np.nonzero(distances)]
            return np.array(list(self.states.values()))[indexes]

        if property == "gettos": 
            adj_mat = self.model.radius_neighbors_graph().toarray()
            gr = nx.from_numpy_matrix(adj_mat)
            return nx.number_connected_components(gr)
